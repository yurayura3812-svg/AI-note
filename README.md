...
| Gradio | 音声入力コンポーネントが標準搭載されており、マイク録音・ファイルアップロードを追加実装なしで実現できる。StreamlitはページリロードベースのUIのため音声処理中の状態管理が複雑になるが、Gradioはその問題がない。Hugging Face Spacesとの親和性も高く無料デプロイが容易 |
